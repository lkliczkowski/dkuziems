...
namespace ImprovedLM
{
    class Program
    {
        ...
        static double desiredAccuracyPar = 99;
        ...
        static void Main(string[] args)
        {
            while (!end)
            {
                ...
                {
                    menu.Remove("Zakończ", End);
                    menu.Add(">> Rozpocznij/kontynuuj działanie sieci", StartLMlearning);
                    menu.Add("Zakończ", End);
                }
        ...
        public static void LMSetConfig()
        {
        ...
        {
            if (configured)
                nT = new ImprovedLMTrainer(dataset, hiddenNodeRatioPar, maxEpochsPar, desiredAccuracyPar);
            else
                nT = new ImprovedLMTrainer(dataset);

            readyToBackpropagate = true;
        }

        public static void StartLMlearning()
        {
            if (nT.TrainNetwork())
            {
